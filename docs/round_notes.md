...
7. 已将远端生成片段从 `set -euo pipefail` 调整为 `set -eu`，减少 Windows OpenSSH 管道执行时的兼容性问题。
8. 已改为先把远端脚本编码为 base64，再通过 SSH 让 VPS 解码执行，避免 Windows PowerShell 5.1 管道改写换行导致 `set` 解析失败。
